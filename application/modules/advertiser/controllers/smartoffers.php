...
class Smartoffers extends CI_Controller {   
    ...
    function list_offers($offset=0){
        ...
        //lọc disable offer
        $disoff = " AND id not in (SELECT distinct  offerid FROM cpalead_disoffer WHERE advid = $uid) ";
        //get offer//
        ...
        $content =$this->load->view('smartoffers/smartoffers.php',$data,true); 
    ...
    function available($offset=0){
        ...
        $data['paymterm'] = $this->Home_model->get_data('paymterm',array('show'=>1));        
        $data['totals'] = $this->total_rows;
        $content =$this->load->view('smartoffers/list_offers.php',$data,true); 
        $this->load->view('default/vindex.php',array('content'=>$content)); 
    }
    function live(){
        $this->phantrang();
        ...
        if($id){
            if($_POST){                        
                ...
                array(
                    ...
                    'offerid'=>$id,
                    'check_trung'=>'smoffers-'.$this->member->id.'-'.$id,
        ...
        if(!$id)
           ...
           if($off){
               ...
               if($off->request){
                   ...
                   $rq=$this->Home_model->get_one('request',array('userid'=>$this->session->userdata('advid'),'offerid'=>$off->id));
                   if(!empty($rq)){
                      $status =  $rq->status;
                   }
               }
               ...
               $content =$this->load->view('smartoffers/campaign_view.php',array('offer'=>$off,'offercat'=>$moffercat,'status'=>$status),true);
           }else{
