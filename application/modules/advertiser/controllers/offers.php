...
class Offers extends CI_Controller {   
    ...
    function available($offset=0){
        ...
        $qr = "SELECT cpalead_offer.* , 'Approved' as status
                FROM cpalead_offer
                WHERE `show` = 1 AND smartoff =0 AND smartlink=0   $ct_Like $cat_Like $oName_Like $where  AND (CASE cpalead_offer.request WHEN 1 THEN (SELECT status From cpalead_request where cpalead_request.offerid =cpalead_offer.id AND cpalead_request.userid = $uid) ELSE 'Approved' END)='Approved'
                ORDER BY `id` DESC 
                LIMIT $offset,$this->per_page
                ";
        $data['offer'] = $this->db->query($qr)->result();
