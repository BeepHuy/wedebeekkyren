...
class Statistics extends CI_Controller {
    ...
    function export_excel(){
        ...
        $where = $data['where'];
        //lấy dữ liệu từ tracklink
        $qr = "SELECT date,oname,countries,device_type,device_model,browser,status,amount2,s1,s2,s3,s4,s5,s6,useragent,id,offerid
          FROM `cpalead_tracklink`  WHERE userid=? AND flead=1 AND date>=? AND date<? $where ORDER BY `id` DESC";
        $data['data'] = $this->db->query($qr,array($this->member->id,$data['from'],$data['to']))->result();
      

        $processedData = array();
        foreach ($data['data'] as $row) {
            $rowArray = (array) $row;  // Chuyển đổi đối tượng thành mảng
            
            // Chuyển đổi giá trị cột status từ số thành chữ
            switch ($rowArray['status']) {
                case 1:
                    $rowArray['status'] = 'Pending';
                    break;
                case 2:
                    $rowArray['status'] = 'Declined';
                    break;
                case 3:
                    $rowArray['status'] = 'Pay';
                    break;
                default:
                    $rowArray['status'] = 'Unknown';
                    break;
            }
             // Nối các trường device_type, device_model, và browser thành một chuỗi duy nhất
             $rowArray['device_type'] = $rowArray['device_type'] . ' ' . $rowArray['device_model'] . ' ' . $rowArray['browser'];
            
             unset($rowArray['device_model']);
             unset($rowArray['browser']);

            $processedData[] = $rowArray;  // Thêm vào mảng dữ liệu đã xử lý
        }
        // Định nghĩa tên cột
        $column_names = array('Date', 'Offer', 'Geography / IP', 'Device', 'Status', 'Income', 'Sub1', 'Sub2', 'Sub3', 'Sub4', 'Sub5', 'Sub6', 'UserAgent', 'ID', 'ID Offers');
        // Xuất dữ liệu ra Excel
        export_to_excel(date('Y-m-d').'_conversions.xlsx', $processedData, $column_names);
    }
    ...
    function referrals(){
        ...
        sum(flead) as lead,count(DISTINCT ip) as hosts
        FROM `cpalead_tracklink`
        WHERE userid IN (SELECT id FROM cpalead_users WHERE ref = {$this->member->id}) and date>=? and date<? $where group by offerid,userid Order BY userid DESC";
    ...
    function locdulieu($type=0){
        ...
        if (!empty($status) && is_array($status)) {
            $dt .= " AND status IN ('" . implode("','", $status) . "')";
        }
    ...
    function ajax_static_dayli(){
        ...
        if($name=='date'){
           ...
           $this->session->set_userdata('to',$arr[1]);
            ...
            $this->session->unset_userdata('sOs');
            $this->session->unset_userdata('dv');
            $this->session->unset_userdata('sCountry');
            $this->session->unset_userdata('Smartlinks');
