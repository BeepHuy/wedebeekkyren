   ...
   <div class="card-body" >
      ...
      <form id="form_loc">
         ...
         <div class="row">
            ...
            <div class="row">
               ...
               ?>
               <div class="col-lg-2 col-sm-6  pe-1 mb-2">
                  <select name="Smartlinks" data-placeholder="Smartlinks" class="chosen-select4 filteroff d-none" multiple tabindex="4">
                     <option value="1">Direct Traffic</option>
                     <option value="2">All Smartlinks</option>
                  </select>
               </div>
               <div class="col-lg-2 col-sm-6  pe-1 mb-2">
                  <select name="status" data-placeholder="Offer Status" class="chosen-select4 filteroff d-none" multiple tabindex="4">
                  <option value="1" <?php echo (is_array($Status) && in_array(1, $Status)) ? ' selected' : ''; ?>>Pending</option>
                  <option value="2" <?php echo (is_array($Status) && in_array(2, $Status)) ? ' selected' : ''; ?>>Declined</option>
                  <option value="3" <?php echo (is_array($Status) && in_array(3, $Status)) ? ' selected' : ''; ?>>Pay</option>
                  </select>
               </div>
            </div>
         </div>
         ...
         $(document).ready(function(){
           //date ranger picker 
            ...
            $('#export').click(function() {              
               window.location.href = '<?php echo base_url('v2/statistics/export_excel'); ?>';
            });
            $('#clear').on('click',function(){
           
               ajaxFilterO(1,'refresh');
            })
      ...
      <div class="border-top mt-3 pt-3 d-flex align-items-center justify-content-between">
         ...
         <div style="position:relative" class="menu_sort">
            <?php if($this->uri->segment(3)=='Conversions'):?>
               <button type="button" id="export" class="btn btn-primary  btn-sm">Export</button>
            <?php endif;?>

            <button type="button" id="clear" class="btn btn-secondary  btn-sm">Clear</button>
            <button type="button" name="refresh" id="refresh" class="btn btn-primary btn-sm">Refresh</button>
         </div>
      </div>
      <!--End chart-->
   </div>
