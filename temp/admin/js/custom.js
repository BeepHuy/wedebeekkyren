$(document).ready(function(){
    ...
    */
    ...
    ///-----------------blocksub2 modal--------------------------------
    $('.block_sub2').click(function(){
        id = $(this).data('id');
        $('.cusermodal').load(adminurl+'blockSub2/'+id);
        $('.userview').modal('show');
    })
    $(document).on('click','.cusermodal button[data-action="blockSub2"]',function(e){
        var url = adminurl+'actionBlockSub2/';
        var sub2data = $('.cusermodal textarea[name="blockSub2"]').val()
         $.ajax({
            type:"POST",
            url:url,
            data:{id:id,sub2data:sub2data},
            success:function(data){
